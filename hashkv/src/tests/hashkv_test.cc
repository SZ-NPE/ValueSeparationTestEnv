...
#define DISK_GB (1024 * 1024 * 1024 * 1)  // 1GB
#define UPDATE_RUNS (3)
...
#define SKIPTHIS                                                           \
  (ki % (ConfigManager::getInstance().getMainSegmentSize() / VALUE_SIZE) % \
       10 >                                                                \
   2)
...
int KVNUM_PER = KVNUM / 100;
ULL DISK_SIZE = 1 * DISK_GB;
bool randkey = true;
int exitCode = 0;
...
void testSetKey(KvServer &kvserver, bool skipSome = false) {
  ...
  for (int i = 0; i < KVNUM; i++) {
    if (i % KVNUM_PER == 0) {
      std::cout << "\r" << i / KVNUM_PER << std::flush;
    }
    int ki = -1;
    ...
    StatsRecorder::getInstance()->timeProcess(
        (runCount > 1 ? StatsType::UPDATE : StatsType::SET), start_time,
        /* diff = */ 0, /* count = */ 1, VALUE_SIZE - ki % VAR_SIZE);
    if (ret == false) {
...
void testReadBackKey(KvServer &kvserver, bool skipSome = false) {
  ...
  for (int i = 0; i < KVNUM; i++) {
    ...
    StatsRecorder::getInstance()->timeProcess(
        StatsType::GET, start_time, /* diff = */ 0, /* count = */ 1, valueSize);
    if (readval == 0) {
      exitCode = -1;
      failed++;
...
void stopTimer(const char *label) { timer(false, label); }
...
int main(int argc, char **argv) {
  ...
  print_yellow(">> Beginning of %s and read-back test",
               "simple fixed-size new write");
  startTimer();
  testSetKey(kvserver);
  stopTimer("SET");
  //  startTimer();
  ...
  //  stopTimer("GET");
  //  print_green(">> End of %s and read-back test", "simple fixed-size new
  //  write");
  ...
  for (int i = 0; i < UPDATE_RUNS; i++) {
    print_yellow(">> Beginning of %s and read-back test (run %d)", "rewrite",
                 i + 1);
    startTimer();
    testSetKey(kvserver, false);
    stopTimer("UPDATE");
    //    testReadBackKey(kvserver, false);
